...
describe('Resource Flow Integration', () => {
    ...
    describe('Backend Resource Discovery', () => {
        ...
        it('should handle resources from multiple backend servers', async () => {
                        ...
                        resources: [
                            {
                                ...
                                name:        'Resource from Server 1',
                            },
                            {
                                originalUri: 'test://resource3',
                                serverName:  'test-server-2',
                                name:        'Resource from Server 2',
                            },
                        ],
                    },
                },
            ...
            expect(resources[0].name).toBe('Resource from Server 1');
            expect(resources[1].name).toBe('Resource from Server 2');
        });
    ...
    describe('Resource Read Proxying', () => {
        it('should apply overrides when proxying resource reads', () => {
            const resources = groupManager.getResourcesForGroup('test-group', mockBackendResources);
            const resource = resources[0];

            // Verify the resource has overrides applied
            expect(resource.name).toBe('Custom Resource Name');
            expect(resource.uri).toBe('test://resource1');

            // The original resource info would be needed for backend read
            const groupConfig = groupManager.getGroup('test-group');
            const resourceOverride = groupConfig?.resources?.[0];
        ...
        it('should handle resource with all overrides applied', async () => {
            const fullOverrideGroup = {
                groups: {
                    'full-override': {
                        name:      'full-override',
                        tools:     [],
            ...
            const fullPath = await createTempConfigFile(fullOverrideGroup);
            const fullManager = new GroupManager(fullPath);
            await fullManager.load();

            const resources = fullManager.getResourcesForGroup('full-override', mockBackendResources);
            const resource = resources[0];

            // Verify all overrides are applied
            expect(resource.name).toBe('Completely Overridden');
    ...
    describe('End-to-End Resource Flow Simulation', () => {
        ...
        it('should handle multiple concurrent resource reads', async () => {
            ...
            expect(resources[1].name).toBe('Resource 2');
            expect(resources[2].name).toBe('Resource 3');
        ...
        it('should maintain resource isolation between groups', async () => {
            ...
            // Same backend resource, different overrides
            expect(resources1[0].name).toBe('Custom Resource Name');
            expect(resources2[0].name).toBe('Different Name');
            ...
            expect(group1?.resources?.[0].originalUri).toBe(group2?.resources?.[0].originalUri);
